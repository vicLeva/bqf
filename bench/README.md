...
- **in-place transcription** (`Bqf::resize_inplace`) — the production path: one
  pass over the old filter, writing remainders + counters straight into the
  grown filter. Linear in element count, needs one extra filter of memory.
- **enumerate + reinsert** (`Bqf::resize_enumerate`) — counter-aware analog of
...
python3 bench/plot_resize.py results.csv resize_compare.png
```
...
under `nohup` or a scheduler. Output (CSV + log + PNG) lands in `OUTDIR`
