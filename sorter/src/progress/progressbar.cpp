...
{
  progressbar* n_prog = (progressbar*)malloc(sizeof(progressbar));
  if(n_prog == NULL) {
    return NULL;
  }
    ...
    n_prog->max = max;
    n_prog->value = 0;
    n_prog->start = time(NULL);
  assert(3 == strlen(format) && "format must be 3 characters in length");
    n_prog->format.begin = format[0];
    n_prog->format.fill = format[1];
    n_prog->format.end = format[2];

  progressbar_update_label(n_prog, label);
  progressbar_draw(n_prog);
